...
SELECT DISTINCT br_dept,br_name
...
CREATE SEQUENCE seq_regist_serial#
    ...
    CACHE 20;--b
UPDATE regist
SET re_serial# = NEXT VALUE FOR seq_regist_serial#;--c

INSERT INTO regist (re_brID, re_crCode, re_semester, re_serial#)
VALUES (10023, 'COMP4201', 'SP2016', NEXT VALUE FOR seq_regist_serial#);

INSERT INTO regist (re_brID, re_crCode, re_semester, re_serial#)
VALUES (3000, 'BIOL1000', 'SP2017', NEXT VALUE FOR seq_regist_serial#);--d

--lab 5 , Q2:
ALTER TABLE CBlink
ADD li_key character(8);--a
...
UPDATE CBlink
SET li_key = LEFT(li_crCode, 4) + RIGHT('0000' + CAST(NEXT VALUE FOR seq_CBlink_key AS VARCHAR(4)), 4);--c

--lab 5 , Q3:
CREATE SEQUENCE seq_CBlink_key START WITH 0 INCREMENT BY 1;
UPDATE CBlink
SET li_key = LEFT(li_crCode, 4) + RIGHT('0000' + CAST(NEXT VALUE FOR seq_CBlink_key AS VARCHAR), 4);
INSERT INTO CBlink (li_crCode, li_bkId, li_usage, li_key)
VALUES ('COMP4201', 1003, 'T', LEFT('COMP' + RIGHT('0000' + CAST(NEXT VALUE FOR seq_CBlink_key AS VARCHAR), 4), 8));

INSERT INTO CBlink (li_crCode, li_bkId, li_usage, li_key)
VALUES ('BIOL1000', 3001, 'R', LEFT('BIOL' + RIGHT('0000' + CAST(NEXT VALUE FOR seq_CBlink_key AS VARCHAR), 4), 8));
